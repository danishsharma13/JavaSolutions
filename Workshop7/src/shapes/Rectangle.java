...
public class Rectangle implements Shape {
	...
	private ShapeArea rectangleArea;
	...
	public Rectangle(double width, double length) throws RectangleException {
		...
		if (length > 0 && width > 0) {
			this.length = length;
			this.width = width;
			// Using lambda expression to create functionality of FunctionalInterface
			// abstract method
	...
	public String toString() {
		...
		return String.format("%s {w=%s, l=%s} perimeter = %g and area = %g", this.getClass().getSimpleName(),
				Double.toString(getWidth()), Double.toString(getLength()), getPerimeter(), rectangleArea.getArea());
	}
}
